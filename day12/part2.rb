...
def find(paths, vertex, path = [], double: false)
  path << vertex.name
  if vertex.name == "end"
    paths << path
  else
    vertex.edges
      .select do |v|
        v.name != "start" &&
          (big?(v) || v.name == "end" || !(path.include?(v.name) && double))
      end
      .each { |v| find(paths, v, path.dup, double: double || (!big?(v) && path.include?(v.name)))  }
  end
end
